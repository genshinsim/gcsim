...
These are canonical role names. The label strings used in this repo's tracker are mapped in `docs/agents/triage-labels.md` — use the strings from that table.
...
_Vendored from the `mattpocock-skills` plugin (`engineering/triage`), adapted for this repo's fork workflow and `docs/agents/` config._
