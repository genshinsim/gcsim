...
Read `docs/agents/issue-tracker.md` for how to fetch the originating issue or PR; it covers this repo's **fork workflow** — issues live on the `upstream` repo, so `gh` commands must be scoped with `--repo genshinsim/gcsim`.
...
Anything in the repo that documents how code should be written, such as `CONTRIBUTING.md`, a `CODING_STANDARDS.md`, or `CONTEXT.md`.
