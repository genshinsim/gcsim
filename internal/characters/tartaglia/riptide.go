...
func (c *char) aimedApplyRiptide(a info.AttackCB) {
...
func (c *char) rangedBurstApplyRiptide(a info.AttackCB) {
	t, ok := a.Target.(*enemy.Enemy)
	if !ok {
		return
	}
...
func (c *char) rtFlashCallback(a info.AttackCB) {
...
func (c *char) rtSlashCallback(a info.AttackCB) {
	// make sure it's actually an enemey
	t, ok := a.Target.(*enemy.Enemy)
	if !ok {
		return
...
func (c *char) rtBlastCallback(a info.AttackCB) {
	// make sure it's actually an enemey
	t, ok := a.Target.(*enemy.Enemy)
	if !ok {
		return
