...
func (c *char) Attack(p map[string]int) action.ActionInfo {
	...
	if c.NormalCounter == c.NormalHitNum-1 {
		...
		if c.female == 0 {
			// Male: A1 comes out pre-hitlag start so it doesn't get delayed by hitlag
			c.Core.QueueAttack(
				ai,
				combat.NewCircleHit(c.Core.Combat.Player(), 0.3, false, combat.TargettableEnemy),
				a1Hitmark[c.female],
			...
			// Female: A1 comes out post-hitlag start so it gets delayed by hitlag
			c.QueueCharTask(func() {
				c.Core.QueueAttack(
					ai,
					combat.NewCircleHit(c.Core.Combat.Player(), 0.3, false, combat.TargettableEnemy),
					a1Hitmark[c.female]-(attackHitmarks[c.female][c.NormalCounter]+1),
					a1Hitmark[c.female]-(attackHitmarks[c.female][c.NormalCounter]+1),
				)
