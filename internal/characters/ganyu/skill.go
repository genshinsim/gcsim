...
func (c *char) Skill(p map[string]int) action.ActionInfo {
	...
	c.Core.QueueAttackWithSnap(ai, snap, combat.NewCircleHitOnTarget(c.Core.Combat.Player(), nil, 4), 13)
	//we get the orbs right away
	c.Core.QueueParticle("ganyu", 2, attributes.Cryo, c.ParticleDelay)

	//flower damage is after 6 seconds
	c.Core.QueueAttackWithSnap(ai, snap, combat.NewCircleHitOnTarget(c.Core.Combat.Player(), nil, 4), 373)
