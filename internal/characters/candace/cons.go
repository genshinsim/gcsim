...
func (c *char) c2() {
	...
	for _, char := range c.Core.Player.Chars() {
		char.AddAttackMod(character.AttackMod{
			Base: modifier.NewBaseWithHitlag("candace-c2", 12*60),
...
func (c *char) c4() {
	...
	charModFunc := func(args ...interface{}) bool {
		...
		c.AddStatMod(character.StatMod{
			Base:         modifier.NewBaseWithHitlag("candace-c4", 12*60),
	...
	c.Core.Events.Subscribe(event.OnCrystallizeCryo, charModFunc, "candace-c4")
	c.Core.Events.Subscribe(event.OnCrystallizeElectro, charModFunc, "candace-c4")
	c.Core.Events.Subscribe(event.OnCrystallizePyro, charModFunc, "candace-c4")
	c.Core.Events.Subscribe(event.OnCrystallizeHydro, charModFunc, "candace-c4")
}
...
func (c *char) c6() {
	for _, char := range c.Core.Player.Chars() {
		...
		this.AddStatMod(character.StatMod{
			Base:         modifier.NewBaseWithHitlag("candace-c6", 12*60),
