...
func (c *char) Attack(p map[string]int) (int, int) {
	...
	for i, mult := range attack[c.NormalCounter] {
		...
		c.Core.Combat.QueueAttack(
			ai,
			core.NewDefCircHit(0.5, false, coretype.TargettableEnemy),
...
func (c *char) burstRestorefunc(a core.AttackCB) {
	if c.Core.Frame > c.restoreICD && c.restoreCount < 5 {
		c.restoreCount++
		c.restoreICD = c.Core.Frame + 60 //once every 1 second
		energy := burstRestore[c.TalentLvlBurst()]
		//apply a4
		excess := int(a.AttackEvent.Snapshot.Stats[core.ER] / 0.01)
		c.coretype.Log.NewEvent("a4 energy restore stacks", coretype.LogCharacterEvent, c.Index, "stacks", excess, "increase", float64(excess)*0.006)
...
func (c *char) swordAttack(f int, a int) (int, int) {
	...
	for i, mult := range attackB[c.NormalCounter] {
		...
		c.Core.Combat.QueueAttack(
			ai,
			core.NewDefCircHit(2, false, coretype.TargettableEnemy),
...
func (c *char) Skill(p map[string]int) (int, int) {
	...
	//activate eye
	c.Core.AddStatus("raidenskill", 1500+f)
...
func (c *char) eyeOnDamage() {
	c.Core.Subscribe(coretype.OnDamage, func(args ...interface{}) bool {
		...
		if c.eyeICD > c.Core.Frame {
			return false
		}
		//ignore if eye not active
		if c.Core.StatusDuration("raidenskill") == 0 {
			return false
		}
		...
		if c.Base.Cons >= 2 && c.Core.StatusDuration("raidenburst") > 0 {
			ai.IgnoreDefPercent = 0.6
		}
		c.Core.Combat.QueueAttack(ai, core.NewDefCircHit(2, false, coretype.TargettableEnemy), 5, 5)

		c.eyeICD = c.Core.Frame + 54 //0.9 sec icd
		return false
	}, "raiden-eye")
...
func (c *char) Burst(p map[string]int) (int, int) {
	...
	c.stacks = 0
	c.Core.AddStatus("raidenburst", 420+f) //7 seconds
	c.restoreCount = 0
	c.restoreICD = 0
	c.c6Count = 0
	c.c6ICD = 0
	...
	c.coretype.Log.NewEvent("resolve stacks", coretype.LogCharacterEvent, c.Index, "stacks", c.stacksConsumed)
...
func (c *char) onBurstStackCount() {
	...
	//a4 stack gain
	particleICD := 0
	c.Core.Subscribe(core.OnParticleReceived, func(args ...interface{}) bool {
		if particleICD > c.Core.Frame {
			return false
		}
		particleICD = c.Core.Frame + 180 // once every 3 seconds
		c.stacks += 2
		if c.stacks > 60 {
			c.stacks = 60
		}
