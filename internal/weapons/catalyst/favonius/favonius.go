...
func weapon(c def.Character, s def.Sim, log def.Logger, r int, param map[string]int) {
	...
	s.AddOnAttackLanded(func(t def.Target, ds *def.Snapshot, dmg float64, crit bool) {
		...
		log.Debugw("favonius codex proc'd", "frame", s.Frame(), "event", def.LogWeaponEvent)

		c.QueueParticle("favonius codex", 3, def.NoElement, 150)

		icd = s.Frame() + cd

	}, fmt.Sprintf("favo-codex-%v", c.Name()))
