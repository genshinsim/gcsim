...
func TestReduce(t *testing.T) {
	r := &Reactable{}
	r.Durability[model.Element_Electric] = 20
	r.reduce(attributes.Electro, 20, 1)
	if r.Durability[model.Element_Electric] != 0 {
		t.Errorf("expecting nil electro balance, got %v", r.Durability[model.Element_Electric])
	}

	// straight up consumption
	r.Durability[model.Element_Fire] = 20
	...
	if r.Durability[model.Element_Fire] != 25 {
...
func TestTick(t *testing.T) {
	...
	if trg.Durability[model.Element_Electric] != 0.8*25 {
		t.Errorf("expecting 20 electro, got %v", trg.Durability[model.Element_Electric])
	}
	if trg.DecayRate[model.Element_Electric] != 20.0/(6*25+420) {
		t.Errorf("expecting %v decay rate, got %v", 1.0/(6*25+420), trg.DecayRate[model.Element_Electric])
	}
	...
	// test multiple aura
	trg.Durability[model.Element_Electric] = 0 // reset from previous test
	trg.DecayRate[model.Element_Electric] = 0
	trg.AttachOrRefill(&combat.AttackEvent{
		Info: combat.AttackInfo{
			Element:    attributes.Electro,
			Durability: 50,
	...
	// make sure > 0
	if trg.Durability[model.Element_Electric] < 0 {
		t.Errorf("expecting electro not to be 0 yet, got %v", trg.Durability[model.Element_Electric])
	}
	...
	trg.Durability[model.Element_Frozen] = 50
	for i := 0; i < 208; i++ {
		trg.Tick()
	...
	// should be > 0 still
	if trg.Durability[model.Element_Frozen] < 0 {
		t.Errorf("expecting frozen not to be 0 yet, got %v", trg.Durability[model.Element_Frozen])
	}
	// 1 more tick and should be gone
	trg.Tick()
	if trg.Durability[model.Element_Frozen] > 0 {
		t.Errorf("expecting frozen to be gone, got %v", trg.Durability[model.Element_Frozen])
	}
	...
	if trg.DecayRate[model.Element_Frozen] < frzDecayCap {
		t.Errorf("expecting frozen decay to > cap, got %v", trg.Durability[model.Element_Frozen])
	}
	// 1 more tick to reset decay
	trg.Tick()
	if trg.DecayRate[model.Element_Frozen] > frzDecayCap {
		t.Errorf("expecting frozen decay to reset, got %v", trg.Durability[model.Element_Frozen])
	}
}
...
func (target *testTarget) allNil(t *testing.T) bool {
	...
	for i, v := range target.Durability {
		ele := attributes.ModifierToElement[model.Element(i)]
