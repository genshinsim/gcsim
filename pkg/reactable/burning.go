...
func (r *Reactable) TryBurning(a *combat.AttackEvent) bool {
	...
	dendroDur := r.Durability[model.Element_Grass]
	...
	case attributes.Pyro:
		...
		if r.Durability[model.Element_Grass] < ZeroDur && r.Durability[model.Element_Overdose] < ZeroDur {
			return false
		}
	...
	if r.Durability[model.Element_BurningFuel] < ZeroDur {
		r.attachBurningFuel(max(dendroDur, r.Durability[model.Element_Overdose]), 1)
		r.attachBurning()
...
func (r *Reactable) attachBurningFuel(dur, mult reactions.Durability) {
	...
	r.Durability[model.Element_BurningFuel] = mult * dur
	decayRate := mult * dur / (6*dur + 420)
	if decayRate < 10.0/60.0 {
		decayRate = 10.0 / 60.0
	}
	r.DecayRate[model.Element_BurningFuel] = decayRate
}
...
func (r *Reactable) burningCheck() {
	if r.Durability[model.Element_Burning] < ZeroDur && r.Durability[model.Element_BurningFuel] > ZeroDur {
		// no more burning ticks
		r.burningTickSrc = -1
		// remove burning fuel
		r.Durability[model.Element_BurningFuel] = 0
		r.DecayRate[model.Element_BurningFuel] = 0
	}
}
