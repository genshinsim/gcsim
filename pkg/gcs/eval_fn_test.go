...
func TestFib(t *testing.T) {
	...
	if result.Typ() != typRet {
		t.Errorf("expecting type to return ret, got %v", result.Typ().String())
	}
	if eval.Err() != nil {
		t.Error(eval.Err())
	}
	...
	if !ok {
		t.Errorf("expecting number for return, got %v", res.res.Typ().String())
		t.FailNow()
	}
	if val.ival != 34 {
...
func TestNestedActions(t *testing.T) {
	...
	if result.Typ() != typNum {
		t.Errorf("expecting type to return num, got %v", result.Typ().String())
	}
	if eval.Err() != nil {
		t.Error(eval.Err())
	}
