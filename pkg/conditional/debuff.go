...
func evalDebuff(c *core.Core, fields []string) int64 {
	//.debuff.res.t1.name
	if len(fields) < 4 {
		c.Log.NewEvent("bad debuff conditon: invalid num of fields", glog.LogWarnings, -1, "fields", fields)
		return 0
	}
	...
	if err != nil {
		//invalid target
		c.Log.NewEvent("bad debuff conditon: invalid target", glog.LogWarnings, -1, "fields", fields)
	}
	...
	if !ok {
		c.Log.NewEvent("bad debuff conditon: target not an enemy", glog.LogWarnings, -1, "fields", fields)
		return 0
	}
