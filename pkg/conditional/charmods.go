...
func evalCharMods(c *core.Core, fields []string) int64 {
	//.mods.bennett.buff==1
	if len(fields) < 3 {
		c.Log.NewEvent("bad char mod conditon: invalid num of fields", glog.LogWarnings, -1).Write("fields", fields)
		return 0
	}
	name := strings.TrimPrefix(fields[1], ".")
	key, ok := shortcut.CharNameToKey[name]
	if !ok {
		c.Log.NewEvent("bad char mod conditon: invalid character", glog.LogWarnings, -1).Write("fields", fields)
		return 0
	}
