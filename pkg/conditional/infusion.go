...
func evalInfusion(c *core.Core, fields []string) int64 {
	...
	if len(fields) < 3 {
		c.Log.NewEvent("bad infusion conditon: invalid num of fields", glog.LogWarnings, -1).Write("fields", fields)
		return 0
	}
	name := strings.TrimPrefix(fields[1], ".")
	key, ok := shortcut.CharNameToKey[name]
	if !ok {
		c.Log.NewEvent("bad infusion conditon: invalid character", glog.LogWarnings, -1).Write("fields", fields)
		return 0
	}
