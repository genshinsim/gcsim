...
func (r *SimulationResult) PrettyPrint() string {
	...
	fmt.Fprintf(&sb, "Average duration of %.2f seconds (min: %.2f max: %.2f std: %.2f)\n",
		*r.Statistics.Duration.Mean, *r.Statistics.Duration.Min,
		*r.Statistics.Duration.Max, *r.Statistics.Duration.SD)
	fmt.Fprintf(&sb, "Average %.2f damage over %.2f seconds, resulting in %.0f dps (min: %.2f max: %.2f std: %.2f) \n",
	...
	fmt.Fprintf(&sb, "Simulation completed %v iterations\n", r.Statistics.Iterations)
