...
function DBView(props: DBViewProps) {
...
export function DbChar({ char }: { char: string }) {
  let { t } = useTranslation();
  ...
  React.useEffect(() => {
    ...
    axios
      .get(url)
      .then(resp => {
        const data = resp.data as DBItem[];
        const dataWhichHasChar = data.filter(({team}) => team.some(c => c.name === char))
        setData(dataWhichHasChar);
        parseFilterUrl();
        setLoading(false);
      })
