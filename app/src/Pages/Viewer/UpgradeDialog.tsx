import { Button, Callout, Classes, Dialog, FormGroup, Intent, NumericInput } from "@blueprintjs/core";
import { useEffect, useState } from "react";
import { useLocation } from "wouter";
import { RootState, useAppDispatch, useAppSelector } from "~src/store";
import { pool, setTotalWorkers } from "../Sim";
import { SimResults, Version } from "./SimResults";
...
// TODO: translations
export default ({ data, redirect, setResult, setError, id }: Props) => {
    ...
    <Dialog
      ...
      <div className="flex justify-between items-end gap-16 mx-4">
        ...
        <div className="flex justify-end gap-[10px]">
          <UpgradeButton cfg={data.config_file} setResult={setResult} setError={setError} />
          <CancelButton
              mismatch={mismatch}
              setOpen={setOpen}
              major={data.schema_version?.major}
...
const DialogBody = ({ mismatch, major, minor }:
    { mismatch: MismatchType | null, major?: number, minor?: number }) => {
  const VersionInfo = ({}) => (
  ...
  return (
    <Callout title="Major Version Mismatch" intent={Intent.DANGER}>
      <div>
        Simulation results are incompatible with latest version of gcsim. Upgrade will
        attempt to resimulate and generate new results.
      </div>
      <VersionInfo />
    </Callout>
  );
...
const UpgradeButton = ({ cfg, setResult, setError }: {
      cfg?: string,
      setResult: (result: SimResults | null) => void,
      setError: (err: string | null) => void
    }) => {
...
const CancelButton = ({ mismatch, setOpen, major, redirect, id }: {
  ...
  if (major == null && id != null) {
    return (
      <Button
          text="Legacy Viewer"
          onClick={() => setLocation("/legacy/viewer/share/" + id)} />
    );
  }
