...
| C4 | Partial | Implemented in [internal/characters/nefer/cons.go](internal/characters/nefer/cons.go), including the Verdant Dew gain-rate boost and the 4.5s lingering Dendro RES shred refresh model, but the nearby-opponent radius is still approximate. |
| C5 | Done | Covered by `BurstCon = 5` in [internal/characters/nefer/nefer.go](internal/characters/nefer/nefer.go). |
| C6 | Partial | The Lunar-Bloom elevation hook is implemented in [internal/characters/nefer/cons.go](internal/characters/nefer/cons.go), and the second-stage replacement hit plus post-Phantasm extra hit are now implemented in [internal/characters/nefer/charge.go](internal/characters/nefer/charge.go), but their exact geometry and timing still use the current Phantasm AoE approximation. |
