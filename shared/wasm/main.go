...
func runSim(this js.Value, args []js.Value) interface{} {
	...
	if len(args) != 1 {
		return js.ValueOf(errToString("invalid no of arguments passed"))
	}

	var in runConfig

	err := json.Unmarshal([]byte(args[0].String()), &in)
	if err != nil {
		return js.ValueOf(errToString(fmt.Sprintf("error parsing json: %v", err)))
	}

	parser := parse.New("single", in.Config)
	cfg, err := parser.Parse()
	if err != nil {
		return js.ValueOf(errToString(fmt.Sprintf("err parsing config: %v", err)))
	}
	...
	if cfg.Mode.Average {
		...
		s, err := runSingle(cfg)
		if err != nil {
			return js.ValueOf(errToString(err.Error()))
		}
		s.Mode = "single"
