...
GOOS=darwin GOARCH=arm64 go build -ldflags="-X 'main.shareKey=${SHARE_KEY}' -X main.version=`git tag --sort=-version:refname | head -n 1`" -o gcsim_darwin_arm64.exe ./cmd/gcsim
GOOS=darwin GOARCH=amd64 go build -ldflags="-X 'main.shareKey=${SHARE_KEY}' -X main.version=`git tag --sort=-version:refname | head -n 1`" -o gcsim_darwin_amd64.exe ./cmd/gcsim
GOOS=linux GOARCH=amd64 go build -ldflags="-X 'main.shareKey=${SHARE_KEY}' -X main.version=`git tag --sort=-version:refname | head -n 1`" -o gcsim_linux_amd64.exe ./cmd/gcsim
