...
:::info As an exception, `print` can take both number and string as arguments. The following is valid:
...
:::info Note that any valid expression that evaluates into a number can also be printed. For example:
...
:::caution The syntax `wait(arg)`, while valid, is deprecated in favour of `sleep(arg)` :::
...
:::danger `arg` must be a number or an expression that evaluates to a number and represents the number of frames the simulator will wait for. :::

:::caution Due to how gcsim handles actions, the current implementation of `sleep` is not intuitive when trying to extend the duration of actions. Please use `delay` for this purpose instead.
...
Many users would expect that gcsim sleeps for 2 frames _after_ Keqing's N1 action ends. This is not how `sleep` works. `sleep` makes the sim sleep for 2 frames _after_ Keqing's N1 action has reached its specified `CanQueueAfter` value. The duration of `sleep` counts towards the action length.
...
:::caution `delay` is executed before the sim checks if the next action is ready.
...
:::caution If the active character is affected by hitlag during the execution of `delay`, then it will last longer than specified.
...
:::danger **THIS FUNCTION IS EXPERIMENTAL AND SUBJECT TO CHANGE.**
...
:::danger The following example is subject to breaking in the future! :::
...
:::danger `arg1` must be a string (wrapped in double quotes) and `arg2` must be a number or an expression that evaluates to a number. :::
...
:::danger `arg` must be a number or an expression that evaluates to a number. :::

:::danger If `arg` is an invalid target (i.e. 3 when there are only 2 targets), then gcsim will exit with an error. :::

:::info For example, if there are 2 targets, then `set_default_target(2)` will set the default target to the 2nd one. Note that it starts at 1 and not 0 because 0 is a special case (target 0 represents the player). :::
...
:::danger `x` and `y` must be a number or an expression that evaluates to a number. :::
...
:::danger All arguments must be a number or an expression that evaluates to a number. :::
...
:::info `element` can also be "any" to pick up the oldest crystallize shard of any element. :::

:::danger `element` must be a string or an expression that evaluates to a string. :::
...
:::danger `func` must be a function or an expression that evaluates to a function. :::
