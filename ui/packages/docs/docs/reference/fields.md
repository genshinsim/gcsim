...
| `keys` | `char`/`weapon`/`artifact`/`element` | char/weapon/artifact/element name | - | Evaluates to the key for the specified char/weapon/artifact/element name. See the relevant character/weapon/artifact page for acceptable names. Element names are `electro`, `pyro`, `cryo`, `hydro`, `dendro`, `quicken`, `frozen`, `anemo`, `geo`, `physical`. |
