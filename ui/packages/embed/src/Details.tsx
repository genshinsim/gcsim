    ...
    <div className="flex flex-col  grow">
      ...
      <div className="flex flex-col ml-2 mr-2 mb-2 mt-2 p-2 rounded bg-[#252A31] ">
        <div className="grid grid-cols-3">
          ...
          <div>{`Sim duration: ${Math.round(data.statistics.duration.mean)}s`}</div>
          <div>Iters: {data.statistics.iterations} </div>
        </div>
        <div className="grid grid-cols-3">
          <div>{`Target count: ${data.target_details.length}`}</div>
          <div>{`DPS: ${Math.round(data.statistics.dps.mean).toLocaleString()}`}</div>
          <div>{`DPS/Target: ${Math.round(data.statistics.dps.mean / data.target_details.length).toLocaleString()}`}</div>
        </div>
      </div>
    </div>
  );
