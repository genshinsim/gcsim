...
const totalRows: StatRow[] = [
	...
	{
		key: "atk",
		name: (t) => `${t("stats.atk")} / ${t("stats.atk%")}`,
	...
	{
		key: "def",
		name: (t) => `${t("stats.def")} / ${t("stats.def%")}`,
...
function buildBlocks(
	...
	const values: Record<
		string,
		Record<string, { flat: number; per: number }>
	> = {};
	...
	chars.forEach((char) => {
		...
		rows.forEach((row) => {
			...
			if (
				(stats[row.percentIndex] ?? 0) > 0 ||
				(stats[row.flatIndex] ?? 0) > 0
			) {
				counts[row.key]++;
				rowCount++;
			}
