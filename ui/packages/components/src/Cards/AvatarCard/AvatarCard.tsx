...
type Props = {
    ...
    chars?: (model.ICharacter | null)[];
    invalid?: string[];
    className?: string;
    onImageLoaded?: () => void;
};

export const AvatarCard = ({ chars, invalid, className = "", onImageLoaded = () => { } }: Props) => {
    if (chars?.length === 0) {
        return (
            <div> no data</div>
        )
    ...
    return (
        <div className={"flex flex-row flex-wrap justify-center gap-2" + (className == "" ? "" : " " + className)}>
            {chars?.map((c, i) => {
                let name = ""
                if (c != null) {
                    name = c.name ?? ""
