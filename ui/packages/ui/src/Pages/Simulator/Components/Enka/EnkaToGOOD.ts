...
import {
	type EnkaData,
	FightProp,
	type GenshinItemReliquary,
	type GenshinItemWeapon,
} from "./EnkaTypes";

const charIDToKeyLookup = Object.values(CharDataGen.data).reduce(
	(acc, val) => {
...
const artifactMapByTextMapId: { [key in string]: string } = {};
		...
		enkaData.forEach(
			({
				...
				const result: Character = {
					...
					max_level: Math.max(
						parseInt(propMap["4001"].val) ?? 1,
						ascToMaxLvl(parseInt(propMap["1002"].val) ?? 1),
					),
