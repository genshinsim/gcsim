...
const EnergyWarning = ({ data }: WarningProps) => {
	...
	const [show, setShow] = useState(true);
	const visible =
		show && (data?.statistics?.warnings?.insufficient_energy ?? false);
...
const StaminaWarning = ({ data }: WarningProps) => {
	...
	const [show, setShow] = useState(true);
	const visible =
		show && (data?.statistics?.warnings?.insufficient_stamina ?? false);
...
const IgnoreBurstEnergyMode = ({ data }: WarningProps) => {
	...
	const [show, setShow] = useState(true);
	const visible =
		show && (data?.simulator_settings?.ignore_burst_energy ?? false);
